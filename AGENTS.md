...
* 主动留意并指出以下“坏味道”：
  ...
  * 意图不清晰、抽象混乱、命名含糊；
  * 没有实际收益的过度设计与不必要复杂度。
  * 过度怠于浅显的局部更改（如能使用 `use` 而不使用，而是撰写形如 `std::sync::..` 的完整引用路径）。
* 当识别到坏味道时：
  * 用简洁自然语言说明问题；
...
* 很好，首先我们不是蛆虫，我们不能到像蛆一样在屎山上到处钻洞来达成目的，这只会把屎山弄塌。你不能到处都采用打洞之类的办法来允许特例。
  ...
  * 同样，不能仅仅只停留在「我努力不做 workaround」这一步，应思考如何的架构才能避免需要 workaround 去处理一些问题。
...
* **必须/MUST**使用言简意赅（be brief）的语言风格，除此之外的输出风格无法被接受。
